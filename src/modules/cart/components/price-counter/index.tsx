...
const PriceCounter = React.memo(({ data }: IProps) => (
  <div className="P-cart-price-counter">
    <div className="P-counters">
      ...
      {!!data.discount && <h3>{Settings.translations.discount} <span className="P-G-pink">-{data.discount} {currency}</span></h3>}
      {!!data.promoDiscount && <h3>{Settings.translations.promo_discount} <span className="P-G-pink">-{data.promoDiscount} {currency}</span></h3>}
      {!!data.points && <h3>{Settings.translations.points} <span className="P-G-pink">-{data.points}</span></h3>}
      </div>
