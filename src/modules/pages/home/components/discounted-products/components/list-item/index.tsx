...
const ListItem = React.memo(({ data }: IProps) => {
  ...
  return (
    <Link to={ROUTES.PRODUCTS.DETAILS.replace(':id', `${data.id}`)} className="P-home-discounted-products-list-item">
      ...
      <h3>{data.title}</h3>
      {!!data.discount && <del className="P-discounted-price">{data.price} AMD</del>}
      <span className="P-price">{data.discount ? data.price / 100 * data.discount : data.price} AMD</span>
      <LoaderContent
        loading={cartLoading}
